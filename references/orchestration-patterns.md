...
- "Find security issues in `AuthRepository.kt`" → `security-auditor`
...
```
                    ┌─→ code-reviewer    ─┐
/ship → fan out  ───┼─→ security-auditor ─┤→ merge → go/no-go + rollback + staged rollout
                    └─→ test-engineer    ─┘
```
...
**Examples:** "Find every call site of this deprecated `androidx.lifecycle.LiveData` API across the monorepo," "Summarize what these 30 ADRs say about Hilt module scoping."
...
> *Cold start regressed by ~280 ms in the latest internal release. Crashlytics is clean. The regression shows up on Pixel 8 (Android 15) but not on Galaxy S22 (Android 14). Started after last week's release, which bumped a dozen library versions and rewrote the Hilt application graph.*

Plausible root causes (mutually exclusive, all fit the symptoms):

1. A new SDK is doing synchronous I/O on the main thread during `Application.onCreate`
2. The Hilt graph rewrite shifted an expensive `@Singleton` from lazy to eager initialization
3. The Baseline Profile became invalid after the library bumps and is being regenerated cold
4. An OS-level change in Android 15 throttles a specific API the app uses early in startup
...
```
Cold start regressed by ~280 ms in the latest internal build. Crashlytics
is clean. Repros on Pixel 8 (Android 15), not on Galaxy S22 (Android 14).
Last week we bumped a dozen library versions and rewrote the Hilt graph.

Create an agent team to debug this with competing hypotheses. Spawn three
teammates using the existing agent types:

  - code-reviewer    — investigate Application.onCreate, eager singletons
                       in the new Hilt graph, and any new main-thread I/O
                       added by the dependency bumps
  - security-auditor — investigate any new SDKs added by the bumps that
                       might be doing remote-config / attestation /
                       network calls during startup
  - test-engineer    — propose Macrobenchmark scenarios that would
                       distinguish the hypotheses (cold vs warm vs hot
                       startup, with and without the new singletons),
                       and check whether the existing Baseline Profile
                       is still valid for the new module graph

Have them message each other directly to challenge each other's theories.
Update findings as consensus emerges. Only converge when two teammates
...
4. When `code-reviewer` finds a new analytics SDK initializing eagerly in `Application.onCreate`, it messages `security-auditor` to check whether that SDK is making a network call before consent is captured. `security-auditor` confirms or produces counter-evidence.
5. `test-engineer` proposes a focused Macrobenchmark that toggles the suspect singleton off and measures `timeToInitialDisplay` — the team uses it to verify before declaring consensus.
...
Three Sonnet teammates running for ~10–15 minutes of investigation costs noticeably more than the same three personas spawned as subagents by `/ship`. The justification is *quality of conclusion* — for a release-blocking startup regression where shipping the wrong fix wastes a Play Store rollout cycle, the extra tokens are a bargain. For a routine PR review, stick with `/ship`.
